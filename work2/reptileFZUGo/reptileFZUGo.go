...
`)
	...
	//校外使用:
	//urlClick := "https://info22-443.webvpn.fzu.edu.cn/system/resource/code/news/click/dynclicks.jsp?clickid=" + clicked[0][1] + "&owner=" + owner[0][1] + "&clicktype=wbnews"
...
func goFZU(v string, ch chan int) {
	ann := regexpFZU(v)
	SaveToSQL(ann)
	ch <- 1
}
