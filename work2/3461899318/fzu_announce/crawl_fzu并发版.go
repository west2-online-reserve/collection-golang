...
var (
	...
	lock            sync.Mutex // 保护data的互斥锁
	concurrencyChan = make(chan struct{}, 10)
	db              *sql.DB
)
...
func spider(client *http.Client, url string, wg *sync.WaitGroup) (string, bool) {
	flag := false
	...
	date := document.Find("body > div.wa1200w > div.conth > ul > li:last-of-type > span").Text()
	...
	for i := 0; i < num-1; i++ {
		...
		go get_info(client, announceUrl, wg)
	...
	//return nextUrl
	//下一页url的正则(用于自动翻页)
	link, exit := document.Find("body > div.wa1200w > div.conth > table > tbody > tr > td > a:nth-child(3)").Attr("href")
	if !exit {
		fmt.Println("无下一页")
		return "", true
	}
	...
	// 关键：检查切片是否为空
	if len(link) == 0 {
		fmt.Println("已到达最后一页，没有下一页")
		return "", flag // 返回空字符串表示无下一页
	}
	return public_head + link, flag
}

// 在通知页面提取信息
func get_info(client *http.Client, url string, wg *sync.WaitGroup) {
	defer wg.Done()
...
func main() {
	...
	// 3. 访问目标服务页面
	for {
		nextUrl, flag := spider(client, curUrl, &wg)
		if flag {
			break
		}
		if nextUrl == "" {
			fmt.Println("访问页面为空地址")
			break
		}
		curUrl = nextUrl
	}
	...
	fmt.Println("爬取完成")
	gap := time.Since(std)
	fmt.Printf("并发版时间：%v", gap) //3m57.4190083s
	num := len(data)
	loginMysql()
	for i := 0; i < num; i++ {
		insertTable(&data[i])
